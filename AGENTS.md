...
- **Name:** Snooze Compliance
- **Project ID:** `snooze-compliance-cxmajk`
- **Environment:** prod
...
- Responsive checks: add `device: "phone" | "tablet" | "desktop"` (or a preset name from `ide.list_canvas_devices`, or `width_px` + `height_px`) and `orientation` to `ide.screenshot_canvas`, one call per breakpoint. Switching persists the canvas size like the Builder's picker, so keep the `previous` device the first call returns and `ide.set_canvas_device` back to it when done (by `previous.name`, or by `previous.width_px` + `previous.height_px` when `previous.category` is `custom` — a hand-entered size has no preset name). A `read_only` error means the project has no write access for size changes — drop `device` and critique the current size; an `orientation`-only call still works.
...
- Uploading media assets and referencing them (`sdk.assets.*`, storage paths vs Flutter bundle paths): see "Media assets" below
...
## Media assets (images, fonts, Lottie/Rive, shaders)

Assets are the one part of a project that is NOT in the project proto, so they
cannot be authored in the DSL and pushed. Upload first, then reference what the
upload returns:

```dart
final sdk = FlutterFlowAI(apiKey: apiKey);
final logo = await sdk.assets.uploadFile(projectId, File('logo.png'));
// -> logo.storagePath == 'projects/<projectId>/assets/<folder>/logo.png'

await flutterFlowAI((app) {
  app.page(
    'Home',
    route: '/',
    isInitial: true,
    body: Scaffold(body: Image(logo.storagePath, isNetwork: false)),
  );
}, projectId: projectId, apiKey: apiKey);
```

Also: `sdk.assets.list(projectId)`, `sdk.assets.findByFilename(...)`,
`sdk.assets.uploadFiles(...)`, `sdk.assets.delete(projectId, storagePath)`.

Outside a DSL script, the same operations are available as
`flutterflow ai assets <list|upload|delete>` and as the `assets.list` /
`assets.upload` / `assets.delete` MCP tools.

**Never write a Flutter bundle path.** `assets/images/logo.png` is what the
generated Flutter project ends up containing, but FlutterFlow derives it from
the storage path at build time — the bundle path never appears in a project.
Passing one produces an app that builds and renders nothing, so the compiler
rejects it on asset-sourced `Image` / `Lottie` / `Rive` / `VideoPlayer` /
`PdfViewer` / shader widgets, on `customFont` / `customIconFamily`, and on the
brownfield patch setters (`patch.imagePath`, `patch.pdf`,
`patch.shaderCustomPath`). If you
need an asset and do not have its storage path, upload the file or look it up
with `sdk.assets.findByFilename(...)` — do not invent a path.

**Uploaded assets are public** — served world-readable by URL so generated apps
can load them. Never upload private keys, credentials, or a `.db` seed holding
real user data.

Re-uploading the same file name overwrites in place, so scripts are safe to
re-run. Supported: images, video, audio, `ttf`/`otf`/`woff`, `json` (Lottie),
`riv`, `pdf`, `frag`, `pem`, `db`, `sql`. Max 20 MB. Pass a bare file name, and
one whose extension matches the file — the upload checks the signature, so a
rename can't relabel one kind of file as another.

SVG is not accepted: served publicly it executes when opened. Use PNG or WebP.
The IDE can still upload one, but into the same public bucket with the same
exposure — don't treat that as the safe way round this.
...
- **Pub deps** — pub.dev discovery is your job; the SDK only records the resolution. Declare the dep next to the artifact that imports it. pub.dev names must match `^[a-z][a-z0-9_]*$` (Git/path names only need to be Dart identifiers) and versions cannot be blank. For Git sources use `app.pubGitDependency(name, url:, ref:, path:)` or the brownfield `addPubGitDependency` / `updatePubGitDependency` helpers; `path` selects a package inside a monorepo and a commit SHA is the safest `ref`.
...
app.revenueCat(
  ...
  webPaywallLink: 'abc123',     // the id in https://pay.rev.cat/abc123
...
Actions.revenueCatPaywall(entitlementId: 'premium');  // entitlement check
Actions.revenueCatRestore();

// The RevenueCat-hosted paywall. Reports a bool under outputVariableName:
// purchased, restored, or already entitled. Falls back to the Web Paywall
// link on web, where RevenueCat has no native paywall.
Actions.revenueCatShowPaywall(
  offeringId: 'launch',            // optional; omit for the current offering
  requiredEntitlementId: 'pro',    // optional; skips subscribers entirely
);
Actions.revenueCatOpenWebPaywall();       // needs webPaywallLink
Actions.revenueCatShowCustomerCenter();   // self-service subscription mgmt
Actions.revenueCatManageSubscription();   // the store's own management page

// Customer identity and state
Actions.revenueCatIdentifyUser(appUserId: 'user_42');
Actions.revenueCatLogOut();
Actions.revenueCatSetAttributes(
  email: 'ada@example.com',
  attributes: {'plan_source': 'onboarding'},
);
Actions.revenueCatRefreshCustomerInfo();
Actions.revenueCatSyncPurchases();
```

Variables are read with the `RevenueCat(...)` expression anywhere a DSL value
is accepted. Requires RevenueCat enabled through `app.revenueCat(...)` or
`app.ensureRevenueCat(...)`; compiling one against a project without it
throws:

```dart
Text(RevenueCat(RevenueCatField.appUserId));
Button('Go Pro', visible: Not(RevenueCat(RevenueCatField.hasActiveSubscription)));
SetState('entitlements', RevenueCat(RevenueCatField.entitlementIds));
```

Three of the six `RevenueCatField` values read as a `DslType`, so they bind
into typed state: `appUserId` a string, `hasActiveSubscription` a bool,
`entitlementIds` a list of strings. A `ListView` sourced from
`entitlementIds` therefore gets string items, so read each one with `item`
itself, not `item['id']`.

The other three, `currentOffering`, `allOfferings`, and `customerInfo`, are
RevenueCat SDK objects with no `DslType` equivalent, and **cannot be read
from the DSL yet: do not bind them**. Their contents (a package's price,
currency code, introductory offer, period and price-per-month for "save x%"
copy; an entitlement's `Is Active`, `Will Renew`, `Is In Trial`,
`Has Billing Issue` and its dates; active subscriptions, expiration and
purchase dates, the store management URL) are reached only through the
editor's attribute reducers, which have no DSL expression, and no
`app.raw(...)` path either: attaching a reducer needs variable-operation
builders the package does not export. So don't go looking. Read subscription
state from `hasActiveSubscription` and `entitlementIds`, and render prices
with `Actions.revenueCatShowPaywall(...)`, which draws the current offering
itself.
