...
[**▶ Try the live demo**](https://kya-iota.vercel.app)
...
```bash
flutter test test/widget_test.dart test/visual_test.dart
...
The live demo is `build/web` on Vercel:
...
The browser needs HTTPS for the camera stage. Vercel gives you that.

## Landing page

A React + Vite marketing page lives in [`landing/`](landing/).
...
KYA also exists as a FlutterFlow project (`snooze-compliance-cxmajk`). Its generated SDK lives in `lib/flutterflow_project/`, and the build scripts live in `dsl/`, with examples in `references/` and `patterns/`. [`AGENTS.md`](AGENTS.md) documents how to use them.

It uses a different pubspec. Swap it in before running `flutterflow ai` commands:

```bash
cp flutterflow.pubspec.yaml pubspec.yaml
cp flutterflow.pubspec.lock pubspec.lock
```

Run `git checkout pubspec.yaml pubspec.lock` to switch back to the Flutter app. The Flutter app ignores these files, and `test/app_test.dart` only runs under the FlutterFlow pubspec.
...
Issues and PRs welcome. Run the Development commands above before opening one.
